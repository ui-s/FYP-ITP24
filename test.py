...
def login():
    ...
    if username in users_df.index:
        ...
        try:
            ...
            session['age'] = user_data['Age']  # Changed from_age_group to age
...
def process_step1():
    ...
    session['age'] = request.form.get('age', '')  # Updated to age
...
def ensure_file_termination():
    ...
    if os.path.exists(filename):
        ...
        with open(filename, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Username', 'Gender', 'Age', 'Height_cm', 'Weight_kg', 'Body_Goal', 'Problem_Areas', 'Fitness_Level', 'Workout_Days'])
...
def generate_workout_plan():
    if request.method == 'GET':
        ...
        required_fields = ['username', 'gender', 'age', 'height', 'weight', 'body_goal', 'problem_areas', 'fitness_level', 'workout_days']
        if all(field in session for field in required_fields):
            try:
                user_data = {
                    'Username': session['username'],
                    'Gender': session['gender'],
                    'Age': session['age'],  # Updated to age
        ...
        try:
            ...
            user_data = {
                ...
                'Age': request.form.get('age', ''),  # Updated to age
...
def generate_workout(user_data):
    try:
        ...
        user_input = {
            ...
            'Age': safe_int(user_data.get('Age', ''), default=30),
