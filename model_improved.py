...
class WorkoutModel:
    def __init__(self):
        ...
        self.workout_types = ['Chest Day', 'Back Day', 'Leg Day', 'Arm Day', 'Upper Body', 'Lower Body', 'Cardio', 'Full Body Day']
    ...
    def load_data(self):
        try:
            ...
            logger.info(f"Unique workout types in dataset: {self.workout_exercises_data['WorkoutDay'].unique()}")
    ...
    def optimize_workout_schedule(self, workout_days, predictions):
        ...
        if workout_count < workout_days:
            ...
            for day in additional_workouts:
                schedule[day] = random.choice(self.workout_types)
        ...
        for day in days:
            if schedule[day] != 'Rest':
                ...
                while schedule[day] in used_workouts and attempts < 10:
                    ...
                    attempts += 1
                if attempts == 10:
                    schedule[day] = 'Full Body Day'  # Fallback to Full Body Day if all workout types are used
    ...
    def get_exercises_for_workout(self, workout_type, fitness_level, problem_areas):
        try:
            logger.info(f"Fetching exercises for {workout_type} at {fitness_level} level")
            ...
            if suitable_exercises.empty:
                logger.warning(f"No exercises found for {workout_type} at {fitness_level} level. Trying adjacent levels.")
                adjacent_levels = ['beginner', 'intermediate', 'advanced']
                current_level_index = adjacent_levels.index(fitness_level.lower())
                
                if current_level_index > 0:
                    lower_level = adjacent_levels[current_level_index - 1]
                    suitable_exercises = pd.concat([suitable_exercises, self.workout_exercises_data[
                        (self.workout_exercises_data['WorkoutDay'].str.lower() == workout_type.lower()) &
                        (self.workout_exercises_data['FitnessLevel'].str.lower() == lower_level)
                    ]])
                
                if current_level_index < 2:
                    upper_level = adjacent_levels[current_level_index + 1]
                    suitable_exercises = pd.concat([suitable_exercises, self.workout_exercises_data[
                        (self.workout_exercises_data['WorkoutDay'].str.lower() == workout_type.lower()) &
                        (self.workout_exercises_data['FitnessLevel'].str.lower() == upper_level)
                    ]])
            ...
            suitable_exercises = suitable_exercises.drop_duplicates(subset=['Exercise'])
